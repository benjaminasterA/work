...
class ChefState(TypedDict):
    ...
    # Annotated와 operator.add를 사용하면 기존 대화에 새 대화가 계속 합쳐집니다.
    messages: Annotated[list[str], operator.add]
    # [핵심] 어떤 부서를 방문했는지 '발도장'을 찍어 기록하는 리스트입니다.
    execution_path: Annotated[list[str], operator.add] 
    #추가
    scores: Annotated[list[int], operator.add] # 각 부서 점수 기록
    #############################################################
...
def draw_path_map(path_list,score_list): #score_list 추가
    ...
    # 800x150 크기의 하얀색 도화지를 만듭니다.
    img = Image.new('RGB', (800, 150), color=(255, 255, 255))
    d = ImageDraw.Draw(img) # 붓을 준비합니다.
    
    # 한글이 깨지지 않게 폰트를 설정합니다.
    ...
    for i, node_name in enumerate(path_list):
        ...
        x += 200 # 다음 상자를 위해 가로 위치를 옆으로 옮깁니다.
    
    # 4. 완성된 이미지를 컴퓨터가 읽을 수 있는 바이트 데이터로 변환합니다.
    buf = BytesIO()
    img.save(buf, format="PNG")
    return buf.getvalue() # 그림 데이터를 반환합니다.
...
st.write("에이전트들이 어떻게 협업했는지 로드맵으로 확인해보세요.")

if st.button("🚀 협업 시스템 가동"):
    # 비어있는 게시판을 들고 업무를 시작(invoke)합니다.
    result = app.invoke({"messages": [], "execution_path": [],"scores": []}) # 업무 기록 출력
    #######################################################################################
    # 1. 텍스트로 된 업무 기록을 출력합니다.
    st.subheader("📝 업무 기록 일지")
    for msg in result["messages"]:
        st.info(msg) # 성과 점수 합산 출력
    ...
    # 게시판에 기록된 'execution_path,scores'를 가져와 그림을 그립니다.
    ##############################################################################
    path_img_data = draw_path_map(result["execution_path"], result["scores"]) #추가
